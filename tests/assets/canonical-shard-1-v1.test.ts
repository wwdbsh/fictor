...
const tempManager = createOwnedTempManager("canonical-shard-1-v1");
...
function fixture(): { plan: T015CanonicalShardPlan; presentation: T015DisclosurePresentationEvidence; approval: T015ApprovalEvidence } {
  if (!authorizationRoot) {
    authorizationRoot = tempManager.create("fictor-t015-authorization-", "suite");
...
function v2RecoveryFixture(): { root: string; plan: ReturnType<typeof buildT015V2CanonicalShardPlan>; pins: T015V2LegacySourcePins; jobs: Array<{ index: number; job_id: string; status: "queued" }> } {
  const root = tempManager.create("fictor-t015-v2-recovery-");
...
describe("T015 CANONICAL shard 1 preparation", () => {
  ...
  test("builds deterministically from only pinned sources in a clean temporary checkout root", () => {
    const root = tempManager.create("fictor-t015-clean-");
  ...
  test("requires the root-created post-disclosure approval attestation and binds every approval input", () => {
    ...
    const absentRoot = tempManager.create("fictor-t015-absent-approval-"); copyFixtureSources(absentRoot); const absentPlan = buildT015CanonicalShardPlan(absentRoot); expect(() => buildT015ApprovalEvidence(absentRoot, absentPlan, buildT015RiskDisclosure(), buildT015ProviderSchemaEvidence(), buildT015DisclosurePresentationEvidence(absentRoot, absentPlan, buildT015RiskDisclosure(), buildT015ProviderSchemaEvidence()), new Date("2026-08-12T03:03:00.000Z"))).toThrow(); expect(isT015Authorized(absentRoot, absentPlan)).toBe(false);
    const falseRoot = tempManager.create("fictor-t015-false-approval-"); copyFixtureSources(falseRoot); const falsePlan = buildT015CanonicalShardPlan(falseRoot); const risk = buildT015RiskDisclosure(); const schema = buildT015ProviderSchemaEvidence(); const falsePresentation = buildT015DisclosurePresentationEvidence(falseRoot, falsePlan, risk, schema);
  ...
  test("requires one exact cost result for every request", () => {
    const root = tempManager.create("fictor-t015-costs-"); const { plan, presentation, approval } = fixture(); const batch = plan.batches[0];
    runT015OpsInternal(["init"], root, plan, presentation, approval);
    const preflight = runT015OpsInternal(["preflight-request", "--batch", batch.id, "--observed-at", "2026-08-12T03:03:00.000Z"], root, plan, presentation, approval) as { requests: unknown[] };
  ...
  test("rejects duplicate per-request provider timestamps before paid prepare", () => {
    const root = tempManager.create("fictor-t015-cost-time-"); const { plan, presentation, approval } = fixture(); const batch = plan.batches[0]; runT015OpsInternal(["init"], root, plan, presentation, approval);
    const preflight = runT015OpsInternal(["preflight-request", "--batch", batch.id, "--observed-at", "2026-08-12T03:03:00.000Z"], root, plan, presentation, approval) as { requests: unknown[] };
  ...
  test("authorization fails closed without creating a journal in an isolated root lacking approval artifacts", () => {
    const root = tempManager.create("fictor-t015-no-approval-init-"); copyFixtureSources(root); const plan = buildT015CanonicalShardPlan(root); expect(isT015Authorized(root, plan)).toBe(false); expect(() => readFileSync(resolve(root, T015_JOURNAL_PATH))).toThrow();
  });

  test("makes SUBMITTING durable, emits one exact batch envelope, and never offers a retry", () => {
    const root = tempManager.create("fictor-t015-"); const prepared = prepareFirst(root); expect((prepared.envelope.requests as unknown[]).length).toBe(12);
  ...
  test("persists definite job IDs but fail-stops partial and optional-signal responses", () => {
    for (const mode of ["partial", "signal"] as const) {
      const root = tempManager.create(`fictor-t015-${mode}-`); const prepared = prepareFirst(root); const assets = prepared.plan.assets.slice(0, mode === "partial" ? 11 : 12);
  ...
  test("allows recovery-only polling of definite partial-submit jobs but can never complete the batch", async () => {
    const root = tempManager.create("fictor-t015-partial-recovery-"); const prepared = prepareFirst(root); const definiteAssets = prepared.plan.assets.slice(0, 3);
  ...
  test.each(["timeout", "download"] as const)("preserves the original partial terminal and definite jobs across %s recovery failure and reload", async (mode) => {
    const root = tempManager.create(`fictor-t015-partial-${mode}-`); const prepared = prepareFirst(root); const assets = prepared.plan.assets.slice(0, 3); const jobs = assets.map((asset) => ({ index: asset.index, job_id: `partial-${asset.index}`, status: "queued" })); const responsePath = json(root, "partial.json", { submitted_count: 3, failed_count: 9, jobs });
    expect(() => runT015OpsInternal(["response", "--batch", prepared.batch.id, "--file", responsePath, "--observed-at", "2026-08-12T03:07:00.000Z"], root, prepared.plan, prepared.presentation, prepared.approval)).toThrow("PARTIAL_OR_MISMATCHED");
    runT015OpsInternal(["recovery-open", "--batch", prepared.batch.id, "--operator-phrase", T015_RECOVERY_OPERATOR_PHRASE, "--observed-at", "2026-08-12T03:08:00.000Z"], root, prepared.plan, prepared.presentation, prepared.approval);
  ...
  test.each(["timeout", "download", "model-drift"] as const)("keeps a complete submission reloadable and recovery-only after %s failure", async (mode) => {
    const root = tempManager.create(`fictor-t015-complete-${mode}-`); const submitted = submitFirst(root);
  ...
  test("includes DNS resolution in the per-hop deadline and never fetches after timeout", async () => {
    const root = tempManager.create("fictor-t015-dns-timeout-"); const submitted = submitFirst(root); const response = { all_terminal: true, jobs: submitted.jobs.map((job) => ({ ...job, status: "completed", model: "nano_banana_flash", result_url: `https://cdn.example.com/${job.index}.png` })), summary: jobsSummary(Array(12).fill("completed")) };
    let fetchCalls = 0;
  ...
  test("revalidates DNS and peer identity on every redirect hop", async () => {
    const root = tempManager.create("fictor-t015-redirect-"); const submitted = submitFirst(root); const bytes = png(); const response = { all_terminal: true, jobs: submitted.jobs.map((job) => ({ ...job, status: "completed", model: "nano_banana_flash", result_url: `https://one.example.com/${job.index}.png` })), summary: jobsSummary(Array(12).fill("completed")) };
  ...
  test.each(["peer", "content-type", "oversize", "abort"] as const)("fail-stops secure download on %s violation", async (mode) => {
    const root = tempManager.create(`fictor-t015-download-${mode}-`); const submitted = submitFirst(root); const response = { all_terminal: true, jobs: submitted.jobs.map((job) => ({ ...job, status: "completed", model: "nano_banana_flash", result_url: `https://cdn.example.com/${job.index}.png` })), summary: jobsSummary(Array(12).fill("completed")) };
  ...
  test("rejects future and stale production-clock preflight observations", () => {
    const root = tempManager.create("fictor-t015-clock-"); const { plan, presentation, approval } = fixture(); const now = () => new Date("2026-08-12T04:00:00.000Z"); runT015OpsInternal(["init"], root, plan, presentation, approval, false, now);
  ...
  test("opens recovery only for durable IDs and blocks batch 2 on canary model drift", async () => {
    const root = tempManager.create("fictor-t015-drift-"); const submitted = submitFirst(root); const jobsRequest = runT015OpsInternal(["jobs-request", "--batch", submitted.batch.id], root, submitted.plan, submitted.presentation, submitted.approval); expect(jobsRequest.new_paid_submit).toBe(false);
  ...
  test("redacts stdin URLs, rejects private DNS, and does not submit a replacement", async () => {
    const root = tempManager.create("fictor-t015-ssrf-"); const submitted = submitFirst(root); const response = { all_terminal: true, jobs: submitted.jobs.map((job) => ({ ...job, status: "completed", model: "nano_banana_flash", result_url: `https://cdn.example.com/${job.index}?secret=x` })), summary: jobsSummary(Array(12).fill("completed")) };
  ...
  test.each([
    ...
    const root = tempManager.create("fictor-t015-mapped-"); const submitted = submitFirst(root); const response = { all_terminal: true, jobs: submitted.jobs.map((job) => ({ ...job, status: "completed", model: "nano_banana_flash", result_url: `https://cdn.example.com/${job.index}.png` })), summary: jobsSummary(Array(12).fill("completed")) };
    let fetchCalls = 0;
  ...
  test("recovers provider-native PNGs atomically to distinct local and backup roots", async () => {
    const root = tempManager.create("fictor-t015-recovery-"); const submitted = submitFirst(root); const bytes = png(); const response = { all_terminal: true, jobs: submitted.jobs.map((job) => ({ ...job, status: "completed", model: "nano_banana_flash", result_url: `https://cdn.example.com/${job.index}.png` })), summary: jobsSummary(Array(12).fill("completed")) };
  ...
  test("accepts a public IPv6 pin, preserves hostname/SNI, and canonicalizes peer spelling", async () => {
    const root = tempManager.create("fictor-t015-ipv6-"); const submitted = submitFirst(root); const bytes = png(); const response = { all_terminal: true, jobs: submitted.jobs.map((job) => ({ ...job, status: "completed", model: "nano_banana_flash", result_url: `https://cdn.example.com/${job.index}.png` })), summary: jobsSummary(Array(12).fill("completed")) };
    let fetchCalls = 0;
    const deps: T015DownloadDependencies = {
  ...
  test("checks every CANONICAL index 332+ path in public and backup", () => {
    const root = tempManager.create("fictor-t015-excluded-");
    ...
    const symlinkRoot = tempManager.create("fictor-t015-excluded-link-"); mkdirSync(resolve(symlinkRoot, "public/assets/cards"), { recursive: true }); symlinkSync(resolve(root, "public/assets", excludedPath), resolve(symlinkRoot, "public/assets", excludedPath));
  ...
  test("writes evidence with atomic no-clobber and rejects conflict or symlink targets", () => {
    const root = tempManager.create("fictor-t015-evidence-");
    ...
    const linked = tempManager.create("fictor-t015-evidence-link-"); mkdirSync(resolve(linked, "evidence")); symlinkSync(resolve(root, "evidence/actual.json"), resolve(linked, "evidence/actual.json"));
  ...
  describe("T015 immutable v1 to recovery-only v2 migration", () => {
    test("pins the old failure while splitting 12 recovered plus 320 fresh-paid assets under the original 498 cap", () => {
      ...
      const oldApprovalRoot = tempManager.create("fictor-t015-v2-old-approval-"); copyFixtureSources(oldApprovalRoot); mkdirSync(resolve(oldApprovalRoot, T015_V2_APPROVAL_PATH, ".."), { recursive: true }); copyFileSync(resolve(repositoryRoot, "assets/evidence/t015-forensic-approval-v1.json"), resolve(oldApprovalRoot, T015_V2_APPROVAL_PATH)); expect(isT015V2Authorized(oldApprovalRoot, plan)).toBe(false);
    });
