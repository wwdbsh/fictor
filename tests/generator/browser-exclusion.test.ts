...
describe("canonical catalog browser exclusion", () => {
  ...
  it("keeps generated data and Node-only tooling out of the production bundle", { timeout: 60_000 }, () => {
    const buildDirectory = mkdtempSync(resolve(tmpdir(), "fictor-browser-bundle-"));
    try {
      const build = spawnSync("npm", ["exec", "vite", "--", "build", "--outDir", buildDirectory, "--emptyOutDir"], {
        cwd: repositoryRoot,
        encoding: "utf8",
      ...
      const bundle = filesUnder(buildDirectory)
      ...
      // Budget only the current entrypoint graph. index.html is authoritative,
      // and the isolated outDir cannot race with smoke or another local build.
      const builtFiles = currentIndexAssets(buildDirectory);
