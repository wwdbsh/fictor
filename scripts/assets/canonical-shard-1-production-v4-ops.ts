...
import {
  ...
  T015_V4_3_REMEDIATION_ASSET_COUNT, T015_V4_3_REMEDIATION_BATCH_ID, T015_V4_3_REMEDIATION_CREDIT_UNITS, T015_V4_3_REMEDIATION_INDICES,
  T015_V4_LEGACY_JOURNAL_PATH, T015_V4_LEGACY_LOCK_PATH, T015_V4_ORIGINAL_JOURNAL_PATH,
...
/** v4.1 -> v4.2 migration pin, written once and thereafter carried forward verbatim. */
export interface T015V4MigrationForensics { operations_v4_path: typeof T015_V4_ORIGINAL_JOURNAL_PATH; operations_v4_sha256: string; migrated_at: string }
/** v4.2 -> v4.3 migration pin, added on top of the v4.1 pin so the whole chain stays visible. */
export interface T015V43MigrationForensics { operations_v4_2_path: typeof T015_V4_LEGACY_JOURNAL_PATH; operations_v4_2_sha256: string; migrated_v4_3_at: string }
/**
 * What a migrate accepts as pinned parameters about its source instead of recomputing them:
 * the three header hashes bound to the evidence chain the source was written under, and the
 * batch count of that generation (v4.3 appends the remediation batch, so the source is short
 * by exactly one record).
 */
export interface T015V4PinnedHeader { plan_sha256: string; disclosure_presentation_evidence_sha256: string; approval_evidence_sha256: string; batch_count?: number }
export interface T015V4Journal {
  ...
  immutable_forensics: T015V4Plan["immutable_forensics"] & Partial<T015V4MigrationForensics> & Partial<T015V43MigrationForensics>;
...
/**
 * The authoritative per-index loss ledger, derived from the journal alone: for every batch
 * that booked a loss discharge, the confirmed jobs whose asset never landed anywhere in the
 * run. Recoveries are unioned across all batches, so an index regenerated by the remediation
 * batch drops out of this list automatically.
 */
/**
 * The remediation batch exists to repair losses. A run that lost nothing never needs it, so
 * the exact-COMPLETE path treats an untouched r01 as absent rather than as an open batch.
 */
export function t015V4CompleteWithoutRemediation(journal: T015V4Journal): boolean {
  return journal.batches.every((record) => record.state === "COMPLETE" || (record.batch_id === T015_V4_3_REMEDIATION_BATCH_ID && unstarted(record)));
}
export function t015V4LostIndices(journal: T015V4Journal): Array<{ index: number; asset_id: string; batch_id: string }> {
  const recovered = new Set(journal.batches.flatMap(({ recoveries }) => recoveries.map(({ asset_id }) => asset_id)));
  // An index the remediation batch also failed to deliver was paid for twice but is lost
  // once; the latest batch that tried for it is the one that owns the entry.
...
function assertMigrationPin(path: string | undefined, hash: string | undefined, at: string | undefined, expectedPath: string, label: string): boolean {
  const present = [path, hash, at].filter((value) => value !== undefined).length;
  if (present === 0) return false;
  if (present !== 3 || path !== expectedPath || typeof hash !== "string" || !/^[a-f0-9]{64}$/.test(hash) || typeof at !== "string") throw new Error(`${label} migration forensics changed`);
  timestamp(at);
  return true;
}
function assertForensicsBinding(journal: T015V4Journal, expected: T015V4Journal): void {
  const { operations_v4_path: v4Path, operations_v4_sha256: v4Hash, migrated_at: v4At, operations_v4_2_path: v42Path, operations_v4_2_sha256: v42Hash, migrated_v4_3_at: v42At, ...sources } = journal.immutable_forensics;
  if (canonicalJson(sources) !== canonicalJson(expected.immutable_forensics)) throw new Error("T015 v4 journal header changed");
  const hasV4 = assertMigrationPin(v4Path, v4Hash, v4At, T015_V4_ORIGINAL_JOURNAL_PATH, "T015 v4.2");
  const hasV42 = assertMigrationPin(v42Path, v42Hash, v42At, T015_V4_LEGACY_JOURNAL_PATH, "T015 v4.3");
  // The chain is strictly ordered: a v4.2 pin can only sit on top of the v4.1 pin it succeeded.
  if (hasV42 && !hasV4) throw new Error("T015 v4.3 migration forensics changed");
  if (hasV42 && Date.parse(v42At!) < Date.parse(v4At!)) throw new Error("T015 v4.3 migration forensics changed");
}
...
export function validateT015V4Journal(journal: T015V4Journal, plan: T015V4Plan, presentation: T015V4Presentation, approval: T015V4Approval, runtimeRoot?: string, pinnedHeader?: T015V4PinnedHeader): void {
  ...
  journal.batches.forEach((record, batchIndex) => {
    ...
    if (record.preflight) {
      ...
      if (record.preflight.balance) {
        ...
        // The remediation batch only costs anything if it actually runs, so it is reserved
        // against a preflight balance exactly once: when it is itself the batch preflighting.
        const remaining = plan.batches.slice(batchIndex).reduce((sum, batch, offset) => sum + (batch.id === T015_V4_3_REMEDIATION_BATCH_ID && offset > 0 ? 0 : batch.size * T015_V4_UNIT_COST_UNITS), 0);
  ...
  const runStateIsBad =
    ...
    (journal.run_state === "COMPLETE" && (journal.fail_stop_batch_id !== null || losses !== 0 || !t015V4CompleteWithoutRemediation(journal) || globalRecovered.size !== T015_V4_PAID_ASSET_COUNT || capUsed !== T015_V4_ADDITIONAL_CAP_UNITS)) ||
    // A discharged batch is what makes an exact 480.00/320-asset close impossible, not the
    // money: the provider never charged the failed generations, so every discharge in this
    // run booked 0.00. Closure therefore turns on "something was discharged", not "loss > 0".
    (journal.run_state === "CLOSED_WITH_LOSSES" && (journal.fail_stop_batch_id !== null || !journal.batches.some((record) => lossDischarged(record)) || !allClosable || unresolved.length !== 0));
...
export function runT015V4OpsInternal(args: readonly string[], root: string, plan: T015V4Plan, presentation: T015V4Presentation, approval: T015V4Approval, now?: () => Date): Record<string, unknown> {
  ...
  try {
    if (command === "init") {
      ...
      return { command, run_state: "ACTIVE", observed_at: at, batches: plan.batches.length, paid_assets: T015_V4_PAID_ASSET_COUNT, initial_balance_decimal: anchor.decimal, additional_credit_cap_units: T015_V4_ADDITIONAL_CAP_UNITS, paid_retry_count: 0 };
    }

    // One-shot v4.2 -> v4.3 journal migration. The superseded journal is opened read-only
    // under its own lock, verified in full against its own (pinned) header, and copied
    // verbatim: no batch record, spend, recovery, or terminal is rewritten or replayed. The
    // only addition is the empty remediation batch, whose scope must equal the loss ledger
    // the source journal itself implies.
    if (command === "migrate") {
      const at = timestamp(option(args, "--observed-at"));
      const pinnedHeader: T015V4PinnedHeader = { plan_sha256: option(args, "--legacy-plan-sha256"), disclosure_presentation_evidence_sha256: option(args, "--legacy-presentation-sha256"), approval_evidence_sha256: option(args, "--legacy-approval-sha256"), batch_count: plan.batches.length - 1 };
      if (![pinnedHeader.plan_sha256, pinnedHeader.disclosure_presentation_evidence_sha256, pinnedHeader.approval_evidence_sha256].every((value) => /^[a-f0-9]{64}$/.test(value))) throw new Error("T015 v4.3 migrate requires the three superseded header hashes as sha256 hex");
      if (existsSync(journalPath(root))) throw new Error("T015 v4.3 migrate refuses to clobber an existing journal; the migration runs exactly once");
      ...
      try {
        ...
        if (info.isSymbolicLink() || !info.isFile()) throw new Error("T015 v4.3 migrate source must be a regular file");
        const bytes = readFileSync(absolute);
        const text = bytes.toString("utf8");
        source = JSON.parse(text) as T015V4Journal;
        if (text !== renderT015CanonicalJson(source)) throw new Error("T015 v4.3 migrate source journal is not canonical");
        validateT015V4Journal(source, plan, presentation, approval, root, pinnedHeader);
        sourceSha256 = sha256(bytes);
      } finally { legacyLock.release(); }
      if (source.immutable_forensics.operations_v4_sha256 === undefined) throw new Error("T015 v4.3 migrate source must itself be the migrated v4.2 journal");
      // The disclosed remediation scope is only ever accepted when the settled journal derives
      // exactly the same set of lost indices; otherwise the approval covers the wrong images.
      const lost = t015V4LostIndices(source);
      const remediation = plan.batches.at(-1)!;
      if (remediation.id !== T015_V4_3_REMEDIATION_BATCH_ID) throw new Error("T015 v4.3 plan is missing the remediation batch");
      if (canonicalJson(lost.map(({ index }) => index)) !== canonicalJson([...T015_V4_3_REMEDIATION_INDICES]) || canonicalJson(lost.map(({ asset_id }) => asset_id)) !== canonicalJson(remediation.asset_ids) || lost.length !== T015_V4_3_REMEDIATION_ASSET_COUNT) throw new Error(`T015 v4.3 the journal's lost indices [${lost.map(({ index }) => index).join(",")}] do not match the disclosed remediation scope [${T015_V4_3_REMEDIATION_INDICES.join(",")}]`);
      if (source.batches.some((record) => !settled(source, record) && !unstarted(record))) throw new Error("T015 v4.3 migrate requires every source batch settled or unstarted");
      if (capUsedUnits(source) + T015_V4_3_REMEDIATION_CREDIT_UNITS > T015_V4_ADDITIONAL_CAP_UNITS) throw new Error("T015 v4.3 remediation does not fit inside the approved additional cap");
      ...
      const fresh = buildInitialT015V4Journal(plan, presentation, approval, source.initial_balance);
      const migrated: T015V4Journal = {
        ...fresh,
        immutable_forensics: {
          ...plan.immutable_forensics,
          operations_v4_path: source.immutable_forensics.operations_v4_path, operations_v4_sha256: source.immutable_forensics.operations_v4_sha256, migrated_at: source.immutable_forensics.migrated_at,
          operations_v4_2_path: T015_V4_LEGACY_JOURNAL_PATH, operations_v4_2_sha256: sourceSha256, migrated_v4_3_at: at,
        },
        run_state: source.run_state, fail_stop_batch_id: source.fail_stop_batch_id, resumes: source.resumes,
        batches: [...source.batches, fresh.batches.at(-1)!],
      };
      ...
      return {
        ...
        remediation_batch_id: remediation.id, remediation_indices: lost.map(({ index }) => index), remediation_asset_ids: remediation.asset_ids,
        remediation_credit_decimal: decimal(T015_V4_3_REMEDIATION_CREDIT_UNITS), remaining_cap_units: T015_V4_ADDITIONAL_CAP_UNITS - capUsedUnits(migrated),
    ...
    if (command === "preflight-result") {
      ...
      const remaining = plan.batches.slice(index).reduce((sum, batch, offset) => sum + (batch.id === T015_V4_3_REMEDIATION_BATCH_ID && offset > 0 ? 0 : batch.size * T015_V4_UNIT_COST_UNITS), 0);
    ...
    if (command === "balance-after") {
      ...
      if (!hasActiveTerminal(record)) { transition(record, "COMPLETE", at); if (t015V4CompleteWithoutRemediation(journal)) journal.run_state = "COMPLETE"; }
      writeJournal(context, journal);
...
export function auditT015V4(context: T015V4Context, journal: T015V4Journal, observedAt: string): Record<string, unknown> {
  ...
  const remediation = journal.batches.find(({ batch_id }) => batch_id === T015_V4_3_REMEDIATION_BATCH_ID);
  // Closing is irreversible for the remediation batch: assertOpenForWork needs an ACTIVE run,
  // so promoting while r01 is still runnable would silently forfeit the approved regeneration.
  const stillLost = t015V4LostIndices(journal);
  const remediationStillOwed = remediation !== undefined && unstarted(remediation) && remediation.asset_ids.some((id) => stillLost.some(({ asset_id }) => asset_id === id)) && journal.batches.every((record) => record === remediation || settled(journal, record));
  if (journal.run_state === "ACTIVE" && closable(journal) && discharged.length > 0) {
    if (remediationStillOwed) throw new Error(`T015 v4.3 audit will not close the run while the approved remediation batch ${T015_V4_3_REMEDIATION_BATCH_ID} is unstarted; run it or obtain a new approval to abandon it`);
  ...
  const lost = t015V4LostIndices(journal);
  // A lost index is one that was paid for and never landed; it can never also be recovered,
  // and lost plus recovered can never exceed the paid slice. Unstarted batches are neither.
  if (lost.some(({ asset_id }) => recoveredIds.has(asset_id)) || lost.length + recoveries.length > T015_V4_PAID_ASSET_COUNT) throw new Error("T015 v4.3 per-index loss ledger disagrees with the recovered set");
  // Full delivery via the remediation batch is only consistent at exactly 480.00 / 498.00:
  // every paid image was charged once and every discharge booked exactly what was charged.
  if (recoveries.length === T015_V4_PAID_ASSET_COUNT && (capUsed !== T015_V4_ADDITIONAL_CAP_UNITS || capUsed + T015_V4_LEGACY_COMMITTED_UNITS !== T015_V4_TOTAL_CAP_UNITS)) throw new Error("T015 v4.3 full delivery does not close at 480.00 + 18.00 = 498.00");
  ...
  return {
    ...
    batches: journal.batches.length, paid_batches: T015_V4_PAID_BATCH_COUNT, unstarted_batches: journal.batches.filter((record) => unstarted(record)).length, total_delta_units: capUsed, total_delta_decimal: decimal(capUsed),
    discharged_batches: discharged.length, lost_indices: lost.map(({ index }) => index), lost_index_detail: lost, all_paid_assets_delivered: recoveries.length === T015_V4_PAID_ASSET_COUNT,
    remediation_batch_id: remediation?.batch_id ?? null, remediation_state: remediation?.state ?? null, remediation_recovered: remediation?.recoveries.length ?? 0,
    closes_at_exact_cap: capUsed === T015_V4_ADDITIONAL_CAP_UNITS && capUsed + T015_V4_LEGACY_COMMITTED_UNITS === T015_V4_TOTAL_CAP_UNITS,
    ...
    // No longer an unconditional claim: v4.3 regenerates exactly the approved remediation set.
    contact_segments: segments.length, contact_index_eager_full_image_load: false, regenerated_lost_indices: (remediation?.recoveries.length ?? 0) > 0, regenerated_index_count: remediation?.recoveries.length ?? 0,
  };
}
