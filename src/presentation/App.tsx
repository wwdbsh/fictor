  ...
  return (
    <section className={`canonical-preview ${compact ? "is-compact" : ""}`} aria-label={`레시피 ${canonical.result.nameKo}`}>
      <div className="preview-materials">
        {canonical.materials.map((material, index) => <figure key={material.materialId}><AssetImage src={material.artSrc} placeholderLabel={material.nameKo} alt="" /><figcaption>{material.nameKo}</figcaption>{index === 0 ? <span aria-hidden="true">＋</span> : null}</figure>)}
      </div>
      <span className="preview-equals" aria-hidden="true">＝</span>
      <figure className="preview-result"><AssetImage src={canonical.result.artSrc} fallbackSrc={canonical.materials[0].artSrc} placeholderLabel={canonical.result.nameKo} alt="" /><figcaption><strong>{canonical.result.nameKo}</strong><span>{canonical.result.effectLabelKo}</span>{canonical.result.artFallbackLabelKo ? <small>{canonical.result.artFallbackLabelKo}</small> : null}</figcaption></figure>
  ...
  return (
    <div className="dialog-backdrop">
      <div className="forge-dialog" role="dialog" aria-modal="true" aria-labelledby="forge-dialog-heading" onKeyDown={onKeyDown}>
        ...
        <dl><div><dt>영구 소모 재료 1</dt><dd>{preview.canonical.materials[0].nameKo}</dd></div><div><dt>영구 소모 재료 2</dt><dd>{preview.canonical.materials[1].nameKo}</dd></div><div><dt>결과</dt><dd>{preview.canonical.result.nameKo} · 덱 영구 편입</dd></div><div><dt>연료</dt><dd>{preview.cost.fuelBefore} → {preview.cost.fuelAfter}</dd></div></dl>
...
function ForgePanel({ mode, materials, session, busy, onAction, onClose }: { mode: "WORKSHOP_PAID" | "WORKSHOP_FREE"; materials: readonly Track1UiForgeMaterial[]; session: StillkinTrack1UiSession; busy: boolean; onAction: (action: Track1UiActionDescriptor) => void; onClose?: () => void }) {
  const [selected, setSelected] = useState<string[]>([]);
  const [review, setReview] = useState<Track1UiForgeReview | null>(null);
  const reviewButton = useRef<HTMLButtonElement>(null);
  const preview = selected.length === 2 ? session.previewForge(mode, selected) : null;
...
function JourneyScreen({ projection, session, busy, onAction }: { projection: Extract<Track1UiProjection, { phase: "BETWEEN_NODES" }>; session: StillkinTrack1UiSession; busy: boolean; onAction: (action: Track1UiActionDescriptor) => void }) {
  ...
  return (
    <section className="journey-screen page-screen art-screen">
      <AssetImage className="screen-background" src={projection.backgroundSrc} placeholderLabel="어름의 터" alt="" /><JourneyRail projection={projection} />
...
function CombatCard({ card, handIndex, busy, forgeSelectionMode, selected, onAction, onToggleForge }: { card: Track1UiCard; handIndex: number; busy: boolean; forgeSelectionMode: boolean; selected: boolean; onAction: (action: Track1UiActionDescriptor, handIndex: number) => void; onToggleForge: (instanceId: string) => void }) {
  const content: ReactNode = <><span className="card-cost">{card.cost ?? "—"}</span><strong>{card.nameKo}</strong><AssetImage src={card.artSrc} placeholderLabel={card.nameKo} alt="" />{card.artFallbackLabelKo ? <span className="card-art-note">{card.artFallbackLabelKo}</span> : null}<span className="card-rule">{card.effectLabelKo}</span><span className="card-power">{card.power ?? "—"}</span></>;
...
function CombatScreen({ projection, session, busy, onAction, onCardAction }: { projection: Extract<Track1UiProjection, { phase: "IN_COMBAT" }>; session: StillkinTrack1UiSession; busy: boolean; onAction: (action: Track1UiActionDescriptor) => void; onCardAction: (action: Track1UiActionDescriptor, handIndex: number) => void }) {
  ...
  return (
    <section className="combat-screen page-screen art-screen">
      <AssetImage className="screen-background" src={projection.backgroundSrc} placeholderLabel="어름의 터" alt="" />
      <div className="enemy-stage"><p className="intent-banner">다음 의도 · <strong>{projection.enemy.intentKo}{projection.enemy.intentAmount === null ? "" : ` ${projection.enemy.intentAmount}`}</strong></p><figure className="enemy-record"><AssetImage src={projection.enemy.artSrc} placeholderLabel={projection.enemy.nameKo} alt={projection.enemy.nameKo} /><figcaption><strong>{projection.enemy.nameKo}</strong><span>체력 {projection.enemy.hp} / {projection.enemy.maxHp}</span><span>방어 {projection.enemy.block}</span></figcaption></figure></div>
...
function RewardScreen({ projection, busy, onAction }: { projection: Extract<Track1UiProjection, { phase: "AWAITING_REWARD" }>; busy: boolean; onAction: (action: Track1UiActionDescriptor) => void }) {
  return <section className="reward-screen page-screen"><div className="reward-heading"><h2>전투에서 살아남았습니다.</h2><p>재료 하나를 골라 덱에 넣으세요.</p></div><div className="reward-grid">{projection.choices.map((choice) => <article className="reward-card" key={choice.choiceId}>{choice.artSrc ? <AssetImage src={choice.artSrc} placeholderLabel={choice.nameKo} alt="" /> : <div className="missing-art" aria-hidden="true" />}<h3>{choice.nameKo}</h3><p>{choice.kindLabelKo} · 어름</p><ActionButton action={choice.action} busy={busy} onAction={onAction} /></article>)}</div><StatsStrip projection={projection} /></section>;
}
...
function TerminalScreen({ projection, busy, onAction }: { projection: Extract<Track1UiProjection, { phase: "RUN_WON" | "RUN_LOST" }>; busy: boolean; onAction: (action: Track1UiActionDescriptor) => void }) {
  return <section className={`terminal-screen page-screen ${projection.phase === "RUN_WON" ? "is-won" : "is-lost"}`}><AssetImage src={projection.artSrc} placeholderLabel={projection.headingKo} alt="" /><div><h2>{projection.headingKo}</h2><p>{projection.messageKo}</p><ActionButton action={projection.action} busy={busy} onAction={onAction} className="primary-cta" /></div></section>;
}
