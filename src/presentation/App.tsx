...
function CodexDetail({ entry }: { entry: Track1UiCodexEntry | null }) {
  ...
  return <aside className="codex-detail"><p>No. {String(entry.ordinal).padStart(4, "0")}</p><h3>{entry.preview.result.nameKo}</h3><CanonicalPreview canonical={entry.preview} compact /><div className="discovery-methods"><strong>발견 방법</strong><span>즉석 빚기</span><span>공방 빚기</span><small>두 방식의 발견은 하나의 도감 기록으로 남습니다.</small></div></aside>;
}
...
function CodexSurface({ session, onClose }: { session: StillkinTrack1UiSession; onClose: () => void }) {
  ...
  return (
    <section className="codex-surface" role="dialog" aria-modal="true" aria-labelledby="codex-heading">
...
export function App({ session, initialProjection }: AppProps) {
  ...
  const onAction = (action: Track1UiActionDescriptor) => { if (!pendingAction) setPendingAction(action); };
  const onCombatCardAction = (action: Track1UiActionDescriptor, handIndex: number) => { if (pendingAction) return; combatFocusRequest.current = { focusKey: projection.focusKey, handIndex }; setPendingAction(action); };
  ...
  return (
    <main ref={shell} className={`game-shell phase-${projection.phase.toLowerCase()}`} aria-busy={busy} data-screen-key={projection.screenKey}>
      <ScreenHeader projection={projection} onOpenCodex={() => setCodexOpen(true)} codexButtonRef={codexButton} />
      ...
      {codexOpen ? <CodexSurface session={session} onClose={closeCodex} /> : null}
    </main>
  );
}
