...
export function sha256Hex(input: string): string {
  ...
  const hash: number[] = [...INITIAL_WORDS];
...
export function projectionHash(value: { materials: unknown; inputs: unknown }): string {
  return sha256Hex(canonicalSerialize({ materials: value.materials, inputs: value.inputs }));
}
