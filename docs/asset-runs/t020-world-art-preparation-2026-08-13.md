...
## 9. v1 실행 결과와 v2 (2026-08-13 ~ 14)

### v1 배치 1 결과: 종횡비 canary가 설계대로 작동

배치 `world-art-001`(배경 6장, 16:9, 9.00)은 제출·완료까지 정상이었고 provider가 보고한
model은 6개 모두 `nano_banana_flash`였다(모델 canary 통과). **실패 원인은 모델이 아니라
전달 화소였다.** 첫 자산에서 1376×768이 왔고 이는 정확한 16:9 대비 7813ppm으로 v1 허용치
5000ppm을 넘었다. 규율대로 이미지는 한 장도 저장하지 않고 `ASPECT_MISMATCH`로 정지했다.

- 과금 확인: 363.90 → 354.90, 6장 전부 과금(9.00)
- `acknowledge-loss`로 9.00 확정 기록 → `resume` → `audit` → **CLOSED_WITH_LOSSES**
- v1 저널은 이후 한 바이트도 바뀌지 않는다.

**canary 설계의 값어치:** 이 실패가 배치 1(9.00)에서 났기 때문에 손실이 9.00에서 멈췄다.
적 먼저 순서였다면 같은 결함이 배치 4에서, 이미 54.00을 쓴 뒤에 드러났을 것이다.

### 원인: provider의 32픽셀 격자

provider는 출력 크기를 32픽셀 격자에 맞춘다(1376 = 43×32, 768 = 24×32). 높이 h = 32k인
16:9에서 이상적 너비는 512k/9이고 실제로는 가장 가까운 격자점이 오므로, 비율 오차는
**0.25/k**로 제한된다. 결함이 아니라 크기 정책이며, v1의 허용치가 그 정책을 담기에 좁았다.

| h | k | 전달 w | ppm |
|---|---|---|---|
| 704 | 22 | 1248 | 2,841 |
| 768 | 24 | **1376** | **7,813 ← 관찰** |
| 800 | 25 | 1408 | **10,000** |
| 864 | 27 | 1536 | 0 |

3:4의 관찰값 4445ppm도 같은 현상이며 단지 5000 안쪽이었을 뿐이다.

### v2 허용치: 16:9 12,500ppm

h=800이 정확히 10,000ppm에 떨어지므로 10,000은 경계 위에 앉는 값이고, k≥22의 이론적
최악값 약 11,364ppm을 덮지도 못한다. **12,500**은 격자 메커니즘을 여유 있게 덮으면서
실제 비율이 다른 전달은 그대로 거부한다(7:4 15,625 / 4:3 250,000). 3:4는 5,000 유지.

**의도한 경계:** provider가 반올림 대신 내림을 택했다면 1344×768이 왔을 것이고 그 값은
15,625ppm으로 진짜 7:4 전달과 화소만으로는 구별되지 않는다. v2는 반올림 양자화는 받고
내림은 거부한다 — 정책이 바뀌면 조용히 통과시키지 않고 새 승인을 요구하겠다는 뜻이다.

격자 인식 검사(전달 크기가 목표 비율의 최근접 격자 렌더링인지 확인)가 더 정확하지만
`filesystem.ts` 수정이 필요하고 그 파일은 T015 바인딩에 고정돼 있어 채택하지 않았다.

### v2 범위와 경제

- **무비용 legacy 복구 6장**: v1이 이미 과금한 배경 6장을 job ID로 회수(0 credits).
  **모든 유료 배치보다 먼저** 실행되며, 신규 허용치를 실제 전달물에서 검증하는 검문을 겸한다.
- **유료 48장**: 배경 12 + 적 36, 4개 배치(각 12장), 신규 상한 **72.00**
- v1 기지출 9.00은 되돌릴 수 없지만, 6장을 회수하면 그 9.00은 받은 이미지의 대가가 되어
  **순 금전 손실 0**이 된다. 전부 성공하면 T020 총지출은 9.00 + 72.00 = **81.00**으로
  원래 승인 총액과 같고 54장을 모두 확보한다.
- job ID가 만료돼 회수가 불가능하면 **정지한다.** 유료 재생성으로 자동 전환하지 않는다
  (그러면 상한이 81.00이 되어야 하고 이 승인의 범위가 아니다).

  **운영상 알아둘 것(설계된 동작):** legacy job이 만료돼 정지한 실행은 `run_state`가 ACTIVE인
  채로 남고 마감 경로가 없다. 유료 배치는 legacy 게이트가 막고, 방면할 지출도 없어
  CLOSED_WITH_LOSSES로 갈 수도 없기 때문이다. 이는 결함이 아니라 의도다 — 그 상태에서 할 수
  있는 유일한 올바른 행동은 새 고지와 새 승인을 받는 것이고, 저널은 그때까지 손대지 않은 채
  증거로 남는다. v2 저널을 억지로 닫으려 하지 말 것.

### v2 산출물

- `scripts/assets/t020-world-art-production-v2{,-ops,-cli,-controller}.ts`
- `assets/manifests/t020-world-art-v2.plan.json`, `assets/evidence/t020-world-art-*-v2.json`
- 저널: `assets/runs/t020-world-art/operations-v2.json`
- v1 저널 바이트 사본을 `assets/evidence/t020-world-art-v1-final-journal-forensic.json`으로
  커밋하고 sha256으로 고정했다(`assets/runs/`는 저장소 정책상 gitignore이므로 런타임 경로를
  강제 추가하지 않고 사본을 증거로 남기는 방식을 택했다).
- 유료 진입 게이트(`productionContextT020V2`)는 승인 체인과 committed-clean을 확인하며,
  임시 루트로 직접 구동하는 테스트가 두 게이트의 존재를 고정한다. v1은 소스가 마감된 승인에
  sha로 묶여 있어 같은 방식의 테스트를 추가하지 않았고, 술어 수준에서만 검증한다.
- 감사 보고는 `assets_not_delivered`(미전달)와 `assets_paid_and_lost`(과금 후 상실)를 분리한다.
  v1은 이 둘을 한 숫자로 합쳐 손실을 과장했다.
